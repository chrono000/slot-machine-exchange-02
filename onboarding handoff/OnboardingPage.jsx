...
function LoginScreen({ onSuccess, goTo }) {
  ...
  function handleSubmit(ev) {
    ...
    if (otp && !/^\d{6}$/.test(otp))                 e.otp = "Must be 6 digits";
  ...
  return (
    <div className="ob-screen">
      <div className={"ob-card" + (shaking ? " ob-shake" : "")}>
        <div className="ob-wordmark"><img src="HollaEx network-icon-(L)-01.svg" alt="HollaEx" className="ob-logo" /></div>
        ...
        <form onSubmit={handleSubmit} noValidate>
          ...
          <div className="ob-field">
            <input
              className={"ob-input" + (errors.otp ? " ob-input--error" : "")}
              type="text" inputMode="numeric" maxLength={6}
              value={otp} placeholder="2FA Code (if enabled)"
              onChange={ev => { setOtp(ev.target.value.replace(/\D/g,"")); setErrors(p => ({...p, otp: ""})); }}
              autoComplete="one-time-code" />
            {errors.otp && <span className="ob-err">{errors.otp}</span>}
          </div>
        ...
        <div className="ob-link-row">
          ...
          <button className="ob-link" type="button" style={{opacity:.45,fontSize:"11px",marginTop:"4px"}} onClick={() => { window.location.href = "markets.html"; }}>Markets</button>
        </div>
      </div>
    </div>
  );
